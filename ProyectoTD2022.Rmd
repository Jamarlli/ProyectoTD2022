...
# otras informaciones
n <- df %>% select(recvTime,LAeq,LAeq_d,LAeq_den,LAeq_e,LAeq_n,dateObserved) %>% summarise(Min = min(x),"1st Qu"= quantile(df$LAeq,probs = 0.25,names=F),Median=median(x),Mean=mean(x),"3rd Qu"=quantile(df$LAeq,probs = 0.75,names=F),Max=max(x))



a <- as.data.frame(n) %>% separate(Freq,sep = ":",into = c("a","b")) %>% select(-Var1) %>% mutate(Var2 = as.factor(Var2),a=as.factor(a)) %>%  spread(a, b,-Var2) #%>% kable(col.names = c("c","Min", "1st Qu", "Median","Mean","3rd Qu","Max"),caption = "Summary") %>%  kable_styling()
...
# una idea real
knitr::kable(head(as.data.frame(df)))
```
...
(max <- max(df$Nivel_sonoro)) # No puede haber registros con un Nivel sonoro infinito
pos <- which(df$Nivel_sonoro==Inf) # posicion de los registros con Nivel_sonoro = Inf
...
#Se añaden las etiquetas y el título
  ...
  #Convertimos el grafico en animado
...
g_msonoro <- ggplot(df_msonoro) + #geom_col(aes(x= Dia_medidas, y = Media, fill = Media)) + 
  ...
  # Giramos el angulo de los meses
...
g_estanyo <- df %>% mutate(Dia_medidas = format(Dia_medidas, format="%m-%d")) %>% mutate(Est_anyo = ifelse(Dia_medidas >= "03-20" & Dia_medidas <= "06-21", "Primavera", ifelse(Dia_medidas >= "06-21" & Dia_medidas <= "09-22", "Verano", ifelse(Dia_medidas >= "09-22" & Dia_medidas <= "12-21", "Otoño", ifelse(Dia_medidas >= "12-21" & Dia_medidas <= "12-31" | Dia_medidas >= "01-01" & Dia_medidas <= "03-20", "Invierno", NA))))) %>% unique() %>% 
...
```
Nota: no hay datos de días de mascletà en la ``nombre_estaciones[4]`` (Estacion 5).
