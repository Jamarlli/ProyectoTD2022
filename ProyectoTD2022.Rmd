...
```{r}
df %>% filter(str_detect(Franja_horaria, "h")) %>% mutate(Dia_semana = fct_reorder(weekdays(Dia_medidas), wday(Dia_medidas, week_start = 1))) %>%
  group_by(Franja_horaria,Dia_semana) %>% mutate(Nivel_por_dia = mean(Nivel_sonoro)) %>% 
  #Se seleccionan solo las columnas necesarias
  select(Franja_horaria,Nivel_por_dia, Dia_semana) %>% distinct() %>% 
